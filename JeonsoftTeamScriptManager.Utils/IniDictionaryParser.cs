...
{
    public class IniDictionaryParser
    {
        private List<string> warnings;
        private DictionarySection unSectionedKeyPairs;

        public IniDictionaryParser()
        {
            warnings = new List<string>();
            unSectionedKeyPairs = new DictionarySection("UnSectioned Key Pairs");
        ...
        public DictionarySection UnSectionedKeyPairs
        {
            get { return unSectionedKeyPairs; }
        }

        public Dictionary<string, DictionarySection> Parse(string source)
        {
            string[] lines = source.Split(new string[] { Environment.NewLine }, StringSplitOptions.None);
            Dictionary<string, DictionarySection> sections = new Dictionary<string, DictionarySection>();
            List<string> comments = new List<string>();
            DictionarySection currentSection = null;
            int lineNo = 0;
            foreach (string line in lines)
            {
                ...
                else if (strLine.StartsWith("[") && strLine.EndsWith("]"))
                {
                    ...
                    section = new DictionarySection(strSection) { Comments = comments };
                ...
                else
                {
                    if (currentSection != null)
                    {
                        if (!sections[currentSection.Key].StartsWithAnyOperand(strLine))
                        {
                            ...
                            DictionaryKeyPair pair = new DictionaryKeyPair(sections[currentSection.Key], key, value);
                            pair.Comments = comments;
                            pair.Line = lineNo;
                            pair.KeyPairOperator = strLine[operatorPos];
                            sections[currentSection.Key].Pairs.Add(pair.Key, pair);
                            comments = new List<string>();
                        }
                    }
                    else
                    {
                        ...
                        string key = strLine.Substring(0, operatorPos).Trim();
                        string value = strLine.Substring(operatorPos + 1, strLine.Length - (operatorPos + 1)).Trim();
                        DictionaryKeyPair pair = new DictionaryKeyPair(unSectionedKeyPairs, key, value);
                        pair.Line = lineNo;
                        pair.Comments = comments;
                        pair.KeyPairOperator = keyPairOperator;
                        unSectionedKeyPairs.Pairs.Add(pair.Key, pair);
        ...
        public Dictionary<string, DictionarySection> ParseFile(string fileName)
        {
